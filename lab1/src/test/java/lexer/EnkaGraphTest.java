...
class EnkaGraphTest {
	...
	private void runTest(StringTest test, String regex) {
		runTestWithEnkaAutomata(test, regex);
		runTestWithMEA(test, regex);
	}
	
	private void runTestWithMEA(StringTest test, String regex) {
		...
		assertEquals(test.acceptable, last >= 0);
	}

	private void runTestWithEnkaAutomata(StringTest test, String regex) {
