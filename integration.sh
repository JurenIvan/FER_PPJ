...
time for dir in ${dir_array[*]}
do
    current="./data/lab1/tests/$dir/test"
    echo $current
    cat "$current.lan" | java -cp target/classes lexer.GLA
    cat "$current.in" | java -cp target/classes lexer.LA > output.out
    diff "$current.out" output.out
done
